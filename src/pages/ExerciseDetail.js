...
const ExerciseDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchExercisesData = async () => {
      ...
      const exerciseDetailData = listOfAllExercises.filter(
        (el) => el.id === id
      );
      ...
      const targetMuscleExercisesData = listOfAllExercises.filter((el) =>
        el.target.includes(exerciseDetailData[0].target)
      );
      // console.log(targetMuscleExercisesData)
      setTargetMuscleExercises(targetMuscleExercisesData);

      // const equimentExercisesData = await fetchData(`${exerciseDbUrl}/exercises/equipment/${exerciseDetailData.equipment}`, exerciseOptions);
      const equimentExercisesData = listOfAllExercises.filter(
        (el) => el.equipment === exerciseDetailData[0].equipment
      );
